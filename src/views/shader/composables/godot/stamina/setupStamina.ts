...
export async function setupStamina(
  ...
  const material = new THREE.ShaderMaterial({
    uniforms: {
      ...
      u_changeProgress: { value: params.progress },
      u_changeProgressColor: { value: red },
      u_pi: { value: Math.PI },
    },
    vertexShader,
    fragmentShader,
  ...
  pane
    .addBinding(params, 'changeProgress', {
      label: 'changeProgress',
      min: 0.0,
      max: 1.0,
      step: 0.1,
    })
    .on('change', (ev) => {
      material.uniforms.u_changeProgress.value = ev.value
    })
