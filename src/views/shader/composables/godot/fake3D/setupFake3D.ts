...
export async function setupFake3D(
  ...
  const params={
    ...
    rotZ:0.0,
    scale:1.0,
    pivot:new THREE.Vector2(0.0,0.0)
  }
  ...
  pane.addBinding(params, 'pivot', {
    label: 'pivot',
    x: { min: -2.5, max: 2.5, step: 0.1 },
    y: { min: -2.5, max: 2.5, step: 0.1, inverted: true },
  }).on('change', (ev) => {
    material.uniforms.u_pivot.value.set(ev.value.x, ev.value.y)
  })
