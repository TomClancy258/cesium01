...
import type { MatchedSatellite } from '@/views/aviation-situation/types/satellite'
...
const onDetail = (row: MatchedSatellite) => {
  flyToSatelliteById?.(row.satellite.id)
}
</script>
...
<template>
  <div>
    ...
    <el-table :data="pagedData" border stripe size="small" style="width: 100%">
      <el-table-column prop="satellite.id" label="卫星id" fixed />
      <el-table-column prop="satellite.name" label="卫星名称" />
      <el-table-column prop="satellite.country" label="所属国家" />

      <el-table-column label="纬度">
        <template #default="{ row }">
          {{ formatFixed4(row.satellite.lngLatAlt?.latitude) }}
        </template>
      </el-table-column>
      <el-table-column label="经度">
        <template #default="{ row }">
          {{ formatFixed4(row.satellite.lngLatAlt?.longitude) }}
        </template>
      </el-table-column>
      <el-table-column label="海拔">
        <template #default="{ row }">
          {{ formatFixed4(row.satellite.lngLatAlt?.height) }} m
        </template>
      </el-table-column>
      <el-table-column label="扫描目标">
        <template #default="{ row }">
          {{
            satelliteScanTargetMap[row.satellite.scan?.target] ?? row.satellite.scan?.target
          }}
        </template>
      </el-table-column>
