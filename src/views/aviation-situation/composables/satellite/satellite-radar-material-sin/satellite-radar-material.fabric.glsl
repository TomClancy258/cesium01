...
//  float wave=step(0.88,sinVal);
  ...
  //值越大 → 相邻像素间 sinVal 跳变越剧烈 → 硬阈值切割时明暗交替越突兀 → 锯齿越严重。
  //值越小 → sinVal 在空间上变化平缓 → 即使硬切也不会有明显锯齿。
  //aa=anti-aliasing 抗锯齿
  float aa = max(fwidth(sinVal), 0.002); //0.002它是经验值，大约对应 1~2 个像素在 ST 坐标空间中的宽度，fwidth 在几乎平坦的区域会接近 0。如果软边宽度跟着变成 0，又回到硬切、可能闪/锯。
  //sinVal                       wave
  //< 0.88 - aa                  ≈ 0（暗）
