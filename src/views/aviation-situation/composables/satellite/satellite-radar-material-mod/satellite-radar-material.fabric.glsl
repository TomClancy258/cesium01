...
czm_material czm_getMaterial(czm_materialInput materialInput) {
  ...
  // 右：环 → 缝（段末 fade out）
  //这样upper的抗锯齿就没用，因为1.0+aa已经超过了本坐标范围
  //   float upper = 1.0 - smoothstep(1.0 - aa, 1.0 + aa, spacingXAixs);
  //但下面这种会导致upper即右边的边变细，所以光圈整体变细
  //所以我把光圈从后面[1-thickness,1]移动到中间[0.5,0.5+thickness]【satellite-radar-material-circle-last-position.fabric.glsl里】，
  //但是在spacingXAixs==1.0处会出现很细的光圈，只有把aa固定置为0.002才能解决
  //但是还是推荐用此方法，或者不给upper做fade抗锯齿【最推荐，右边抗锯齿提升不大，不值得维护】，只给lower做抗锯齿，这样光圈就不会变细
//  float upper = 1.0 - smoothstep(1.0 - aa, 1.0, spacingXAixs);
