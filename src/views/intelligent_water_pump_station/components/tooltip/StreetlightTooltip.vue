...
import type { StreetLightRow, TooltipPosition } from '../../types/station-equipment'
...
const { row, visible, toTooltipStyle } = useEquipmentTooltip<StreetLightRow>('streetlight')
...
<template>
  <div v-show="visible" class="equipment-tooltip" :style="tooltipStyle">
    <template v-if="row">
      <div>编号：{{ row.name }}</div>
      <div>名称：{{ row.text }}</div>
      <div>功率：{{ row.power }}</div>
      <div>开关：{{ row.on ? '开' : '关' }}</div>
      <div>
        状态：
